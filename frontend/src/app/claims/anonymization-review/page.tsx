...
export default function AnonymizationReviewQueuePage() {
  const t = useTranslations('anonymization')
  const tClaims = useTranslations('claims')
  const tCountries = useTranslations('countries')
  const tCommon = useTranslations('common')

  const { data, isLoading } = useClaims({ status: 'ANONYMIZATION_REVIEW' })
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {data?.total || 0} claims pending anonymization review
              </CardDescription>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                No claims waiting for anonymization review
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {data?.items.map((claim) => (
                <Link
                  key={claim.id}
                  href={`/claims/${claim.id}/anonymization`}
